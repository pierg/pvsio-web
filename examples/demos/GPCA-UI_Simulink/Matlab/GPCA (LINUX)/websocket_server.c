...
/* DO NOT USE const int EVER, use #define instead.
 Some version of Matlab do not support it and crash with segmentation fault. */
    ...
    if(!force_exit) {
        ...
        switch(callback_reason) {
            ...
            case LWS_CALLBACK_RECEIVE: {
                ...
                gpcauiBus.event = decode_GPCAUI_Command((char*) callback_in);
                if(gpcauiBus.event == E_ConfirmVTBI) {
                    gpcauiBus.programmedVTBI = strtod(((char*) callback_in) + strlen("ConfirmVTBI("), NULL);
                    lwsl_notice("\nprogrammedVTBI = %f\n", strtod(((char*) callback_in) + strlen("ConfirmVTBI("), NULL));
                }
                if(gpcauiBus.event == E_ConfirmDoseRate) {
                    gpcauiBus.programmedDoseRate = strtod(((char*) callback_in) + strlen("ConfirmDoseRate("), NULL);
                }
                
                /* forwarding the commands to the output variables */
                *eventID            = gpcauiBus.event;
                *programmedVTBI     = gpcauiBus.programmedVTBI;
                *programmedDoseRate = gpcauiBus.programmedDoseRate;
                
                /* send actual controller state to the gpcaui */
                write_websocket(callback_wsi,
                                controller_state,
                                isBolusInProgress,
                                totalVolumeInfused,
                                remainingVolumeInReservoir);
...
int main(void) {
    ...
    while (!force_exit) {
        ...
        WebsocketServer(controllerBus.controller_state, controllerBus.isBolusInProgress, controllerBus.totalVolumeInfused, controllerBus.remainingVolumeInReservoir, &a, &b, &c);
